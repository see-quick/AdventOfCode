...
public class day1 {
    ...
    public static void main(String[] args) {
        int sum = 0;
        final String[] lines = puzzle.split("\n");

        for (String line : lines) {
            char[] cArr = line.toCharArray();

            boolean startFlagFound = true;
            boolean endFlagFound = true;
            String resultLine = "";

            for (int i = 0; i < cArr.length; i++) {

                int endPointer = cArr.length - 1 - i;
                int startChar = cArr[i];
                int endChar = cArr[endPointer];

                if (Character.isDigit(startChar) && startFlagFound) {
                    startFlagFound = false;
                    resultLine = Character.getNumericValue(startChar) + resultLine;
                }

                if (Character.isDigit(endChar) && endFlagFound) {
                    endFlagFound = false;
                    resultLine = resultLine + Character.getNumericValue(endChar);
                }
            }
            sum += Integer.parseInt(resultLine);
        }
