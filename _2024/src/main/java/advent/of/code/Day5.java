...
public class Day5 {
    ...
    public static void main(String[] args) {
        //  I want to implement in Java a relation where
        //```
        // 47|53
        // 97|13
        // 97|61
        ...
        //and I want to have
        //
        //47 -> 53 means that 47 can be printed before 53
        //
        //so this. is correct
        //
        //75, 47, 61, 53, 29
        //
        //and I think the best way to implement this would be some relation of time (in this case numbers) and so for each number we would have chain
        //
        //47 -> 53
        //47 -> 13
        //47 -> 61
        ...
        //---
        //
        //so for 47 we would create chain of
        //
        //47 -> 61 -> 53 -> 29 -> 13.
        ...
        final String[] linesOfFirstPart = firstPart.split("\n");
        for (final String line : linesOfFirstPart) {
        ...
        final List<String> correctPathsPapers = new ArrayList<>();
        final String[] linesOfSecondPart = secondPart.split("\n");
        for (final String line : linesOfSecondPart) {
            final String[] numbers = line.split(",");

            for (int i = 0; i < numbers.length - 1; i++) {
                Integer number = Integer.parseInt(numbers[i]);
                Integer nextNumber = Integer.parseInt(numbers[i + 1]);

                List<Integer> containedNumbers = map.get(number);

                if (containedNumbers != null && containedNumbers.contains(nextNumber)) {
                    // correct
                } else {
                    // wrong
                    System.out.println("This is line is wrong (we are gonna not add it):" + line);
                    break;
                }

                // last iteration
                if (i == numbers.length - 2) {
                    // everything is correct so we are gonna add such line
                    correctPathsPapers.add(line);
                }
            }
        }

        System.out.println("Correct paths:" + correctPathsPapers);

        int sum = 0;

        for (String correctPathPaper : correctPathsPapers) {
            String[] numbers = correctPathPaper.split(",");
